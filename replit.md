...
*   **Shepherd.study-Inspired DocChat Redesign (Latest - Simplified Selection Flow)**: Streamlined document selection UX with clean minimal design. Three-column grid layout: left upload panel (file + single unified URL input), center 3-column document grid, right sticky sidebar for selected documents. Selection system: click document cards to select/deselect with purple ring indicator and checkmark, multi-select support with visual feedback. Selected documents sidebar displays chosen docs with count, individual remove buttons, and prominent purple "Start Chat" button (disabled when no selection). Backend auto-detects YouTube vs Website URLs via /api/documents/by-url. Removed text paste option for simplification per user request. All existing features preserved (file upload, URL processing, document management).
