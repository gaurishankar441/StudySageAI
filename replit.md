...
*   **DocChat Session UX Improvements (Latest)**: Fixed YouTube video embed not showing - added videoId to YouTube metadata for proper iframe rendering. Made Quick Actions collapsible with chevron toggle button for better space management. Chat section now gets more vertical space when Quick Actions are collapsed. Fixed PDF controls showing for non-PDF documents - toolbar now conditionally renders only for sourceType='pdf'. Added disabled states to prevent invalid operations: prev/next buttons respect page bounds, zoom buttons constrain to 50-200% range.
