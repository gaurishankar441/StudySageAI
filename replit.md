...
*   **DocChat Session UI Fix (Latest)**: Fixed inappropriate PDF-style controls (page navigation, zoom) appearing for YouTube videos and other non-PDF documents. Now PDF toolbar with page controls only displays for sourceType='pdf'. Added proper disabled states to prevent invalid operations: prev/next buttons respect page bounds, zoom buttons constrain to 50-200% range. YouTube videos now display cleanly with embedded player without PDF navigation clutter.
