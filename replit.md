...
*   **WebSocket Voice Tutor Integration (Latest)**: Implemented real-time voice interaction for AI Tutor using WebSocket protocol at `/tutor/voice`. Features session-based authentication with ownership verification (reject codes 4401/4403/4404), bidirectional audio streaming (AUDIO_CHUNK → STT → TRANSCRIPTION → AI pipeline → TTS_CHUNK), and automatic response triggering on transcription completion. Complete 7-phase AI Tutor pipeline integrated: auto language detection with caching → intent classification (13 types) + emotion detection (5 states, parallel) → session context management → progressive hints (4 Socratic levels) → dynamic prompt generation (language/emotion/phase/misconceptions) → optimized AI response with caching → 4-layer validation → persona-aware TTS streaming (Priya→Anushka, Amit→Abhilash voices) with emotion-based prosody (warm/encouraging/clear). VoiceStreamService coordinates all subsystems: STT (Sarvam Saarika v2, AssemblyAI fallback with upload API for format compatibility), EnhancedVoiceService TTS with math-to-speech conversion and Hinglish code-switching, comprehensive metadata tracking (voiceInput/voiceOutput flags). Message types: AUDIO_CHUNK, TRANSCRIPTION, TTS_CHUNK, TTS_START, TTS_END, INTERRUPT, SESSION_STATE, ERROR, PING, PONG.
*   **Comprehensive Developer Documentation**: Created ultra-detailed technical documentation (`DEVELOPER_DOCUMENTATION.md`) covering complete tech stack, all features and functionality, AI services integration, database schema, API endpoints (method-wise), open source libraries, document processing pipeline, authentication/security, and voice services. Includes code examples, configuration details, and architectural diagrams for full system understanding.
*   **DocChat Session UX Improvements**: Fixed YouTube video embed blocking by updating CSP frameSrc to allow https://www.youtube.com in both production and development modes. Added videoId to YouTube metadata for proper iframe rendering. Made Quick Actions collapsible with chevron toggle button for better space management. Chat section now gets more vertical space when Quick Actions are collapsed. Fixed PDF controls showing for non-PDF documents - toolbar now conditionally renders only for sourceType='pdf'. Added disabled states to prevent invalid operations: prev/next buttons respect page bounds, zoom buttons constrain to 50-200% range. Added scrollable transcript display below YouTube video showing timestamped segments for easy reference.
*   **OCR Image Support with Tesseract.js**: Added optical character recognition (OCR) for image files using Tesseract.js library. Users can now upload image files (PNG, JPG, JPEG, GIF, BMP, WEBP) and the system automatically extracts text content using OCR. Supports English text recognition with confidence scoring. Images are processed as sourceType='image' and routed through extractFromImage method. OCR metadata includes confidence score and language detection (Hindi Devanagari detection included). Proper error handling for images without readable text.
*   **DocChat Redesign - Upload-First Layout**: Redesigned based on user feedback with upload panel as the largest section. Two-column layout: left upload panel (60% width, 3/5 grid) with file upload + unified URL input, right documents panel (40% width, 2/5 grid) with compact list format. Single-selection only - users can select one document at a time. Documents displayed as compact list items with icon, title, status badge, and delete button on hover. Purple border highlights selected document. Start Chat button at bottom shows selection count. Backend auto-detects YouTube vs Website URLs via /api/documents/by-url. Removed text paste option per user request.
